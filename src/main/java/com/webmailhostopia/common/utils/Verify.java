...
public class Verify {
	...
	public static boolean compareStringInNonEnglish(String actual, String expected,Locale locale)throws Exception{
		...
		if(collator.compare(actual.trim(), expected.trim()) == 0){
			Log.info("Actual and Expected string are equal. Actual: "+ actual+ "  Expected: "+expected);
		}else{
			isTrue=false;
			Log.error("Actual and Expected string are not equal. Actual: "+ actual+ "  Expected: "+expected);
		}
		return isTrue;
	}
}
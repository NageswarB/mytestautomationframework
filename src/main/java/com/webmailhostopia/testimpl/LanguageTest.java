...
public class LanguageTest extends AbstractPageObject{
	...
	public static boolean labelsLanguageTestInPreferenceTab() throws Exception{

		List<String> lsStr = new ArrayList<String>();

		String[] sideNavBar = otherLangTestData.getProperty("SIDE_NAV_BAR_MENU_ITEMS_FRENCH").split(",");
		...
		for(WebElement we:ls){
			String tmp = we.getText().trim();
			lsStr.add(tmp);
		}
		String[] actuals = new String[lsStr.size()];
		actuals = lsStr.toArray(actuals);

		boolean testResult1 = Verify.compareStringInNonEnglish(actuals, sideNavBar);
		boolean testResult2 = labelsTestInPreferenceMainActivityPage();
	...
	public static void changeTheLanguage()throws Exception{
		...
		WebElement lan = waitForElementToBeClickable(ByLocator.xpath, "//*[@translate='LANG_PREFERENCES_GENERAL_FORM_LANGUAGE']/../div/a/div[2]/div[2]/div[2]", 30);
		if( lan != null){
	...
	public static boolean labelsTestInPreferenceMainActivityPage()throws Exception{
		
		Log.info("Verfiying labels in preferences tab...");
		
		String expected[] = otherLangTestData.getProperty("LABELS_IN_PREF_TAB_PAGE_FRENCH").split(",");
		...
		boolean stepResult = Verify.compareStringInNonEnglish(actuals, expected);
		return stepResult;
	}
